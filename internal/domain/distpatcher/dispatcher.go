...
//func viewAnswer(domain Apper, showAnswer bool) telebot.HandlerFunc {
...
//		uq, err := domain.GetUserQuestion(ports.GetContext(ctx), ports.GetUserFromContext(ctx).TGUserID, int64(qID))
//		if err != nil {
//			return err
//		}
//
//		// Форматируем текст сообщения
//		question := uq.GetQuestion().QuestionService
//		tag := uq.R.GetQuestion().R.GetTag().TagService
//		answer := uq.R.GetQuestion().R.GetAnswers()[0]
//
//		result := escapeMarkdown(tag) + ": " + escapeMarkdown(question)
//		if showAnswer {
//			result += "\n\n" + escapeMarkdown(answer.AnswerService)
//		}
//
...
//func (d *QuestionDispatcher) NextQuestion(ctx context.Context, user *edu.UserService) (string, error) {
//	// Отправляем подтверждение получения callback
//	if err := ctx.Send(MSG_NEXT_QUESTION); err != nil {
//		return ctx.Respond(&telebot.CallbackResponse{Text: err.Error()})
//	}
//
//	user := ports.GetUserFromContext(ctx)
//
//	// Проверяем время до следующего вопроса
//	t, err := d.domain.GetNearestTimeRepeat(ports.GetContext(ctx), user.TGUserID)
//	if err != nil {
//		return ctx.Respond(&telebot.CallbackResponse{Text: err.Error()})
//	}
//
...
//		Unique: ports.INLINE_FORGOT_HIGH_QUESTION,
//		Text:   MSG_FORGOT,
//		Data:   fmt.Sprintf("%d", uq.QuestionID),
//	}
//
//	easy := telebot.InlineButton{
//		Unique: ports.INLINE_REMEMBER_HIGH_QUESTION,
//		Text:   MSG_REMEMBER,
//		Data:   fmt.Sprintf("%d", uq.QuestionID),
//	}
//
...
//		Unique: ports.INLINE_BTN_REPEAT_QUESTION_AFTER_POLL_HIGH,
//		Text:   label,
//		Data:   fmt.Sprintf("%d", uq.QuestionID),
//	}
//
...
//		Unique: ports.INLINE_BTN_DELETE_QUESTION_AFTER_POLL_HIGH,
//		Text:   BtnDelete,
//		Data:   fmt.Sprintf("%d", uq.QuestionID),
//	}
//
...
//		Unique: ports.INLINE_EDIT_QUESTION,
//		Text:   BtnEdit,
//		Data:   fmt.Sprintf("%d", uq.QuestionID),
//	}
//
...
//			Unique: ports.INLINE_TURN_ANSWER,
//			Text:   "📝 Свернуть ответ",
//			Data:   fmt.Sprintf("%d", uq.QuestionID),
//		}
//	} else {
//		answerBtn = telebot.InlineButton{
//			Unique: ports.INLINE_SHOW_ANSWER,
//			Text:   BtnShowAnswer,
//			Data:   fmt.Sprintf("%d", uq.QuestionID),
//		}
//	}
