...
func testAnswersSoftDelete(t *testing.T) {
	t.Parallel()
...
func testAnswersQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Answers().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testAnswersSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testAnswersQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Answers().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	answerDBTypes = map[string]string{`ID`: `bigint`, `QuestionID`: `bigint`, `Answer`: `text`, `IsCorrect`: `boolean`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_             = bytes.MinRead
)
