...
func testUsersExists(t *testing.T) {
	...
	e, err := UserExists(ctx, tx, o.TGUserID)
	if err != nil {
...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.TGUserID)
	if err != nil {
		t.Error(err)
	}
...
func testUserToManyUsersQuestions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUsersQuestions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUsersQuestions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.TGUserID != first.UserID {
			t.Error("foreign key was wrong value", a.TGUserID, first.UserID)
		}
		if a.TGUserID != second.UserID {
			t.Error("foreign key was wrong value", a.TGUserID, second.UserID)
		}
...
var (
	userDBTypes = map[string]string{`TGUserID`: `bigint`, `ChatID`: `bigint`, `FirstName`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
