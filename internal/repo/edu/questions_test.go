...
func testQuestionsSoftDelete(t *testing.T) {
	t.Parallel()
...
func testQuestionsQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Questions().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testQuestionsSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testQuestionsQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Questions().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testQuestionToManyUsersQuestions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, usersQuestionDBTypes, false, usersQuestionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, usersQuestionDBTypes, false, usersQuestionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UsersQuestions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := QuestionSlice{&a}
	if err = a.L.LoadUsersQuestions(ctx, tx, false, (*[]*Question)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsersQuestions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UsersQuestions = nil
	if err = a.L.LoadUsersQuestions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsersQuestions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testQuestionToManyAddOpUsersQuestions(t *testing.T) {
	var err error
	...
	var a Question
	var b, c, d, e UsersQuestion

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, questionDBTypes, false, strmangle.SetComplement(questionPrimaryKeyColumns, questionColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UsersQuestion{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, usersQuestionDBTypes, false, strmangle.SetComplement(usersQuestionPrimaryKeyColumns, usersQuestionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UsersQuestion{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUsersQuestions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UsersQuestions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UsersQuestions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UsersQuestions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	questionDBTypes = map[string]string{`ID`: `bigint`, `Question`: `text`, `Tag`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
